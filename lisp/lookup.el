...
(defun lookup-init-complement-autoload ()
  ...
  (lookup-foreach
   ...
   (or lookup-complement-autoload-alist
       lookup-support-autoload-alist)))
...
(defun lookup-adjust-content (entry)
  (let ((funcs (lookup-dictionary-adjusts (lookup-entry-dictionary entry))))
    ...
    (let ((overlay (overlay-lists)))
      (mapcar 'delete-overlay (car overlay))
      (mapcar 'delete-overlay (cdr overlay)))
