...
(defun lookup-new-agent (class &optional location &rest options)
  (let* ((agent (lookup-make-agent class location))
    ...
    (lookup-agent-set-options agent (or opts options))
  ...
  (let ((file (lookup-assoc-get lookup-complement-alist
    ...
    (when file
      ...
      (let ((plist (lookup-dictionary-options dictionary))
	    (list (or lookup-complement-options
		      lookup-support-options))) ; to use 1.99 support file
	(while list
...
(defun lookup-dictionary-adjusts (dictionary)
  (or (lookup-dictionary-get-property dictionary 'adjusts)
      (let* ((table1 (lookup-dictionary-option dictionary ':adjust-table))
	     (table2 (lookup-dictionary-ref dictionary ':adjust-table))
	     (adjusts (mapcar (lambda (pair)
				 (or (lookup-assq-get table1 (car pair))
				     (lookup-assq-get table2 (car pair))
				     (cdr pair)))
			       lookup-adjust-table)))
	(lookup-dictionary-put-property dictionary 'adjusts adjusts)
	adjusts)))
